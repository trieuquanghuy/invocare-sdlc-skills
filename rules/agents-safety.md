...
When the dispatched subagent may **write or modify code**, the dispatch prompt MUST additionally cite `.claude/rules/code-quality.md` so the subagent applies the CQ1–CQ13 standards and self-reviews its diff against them — in the form: `Apply .claude/rules/code-quality.md to all code you write.`
